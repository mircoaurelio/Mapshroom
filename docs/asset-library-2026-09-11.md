...
- Background, grayscale gradient, depth and segmentation are selected initially for new preferences, with four matching cards in the library. The Outputs menu adds color gradients and edges, with manual single-zone masks in the surfaces editor. Existing saved output selections are retained. Auto-generation is separate, initially off and saved locally; only new uploaded originals trigger it.
...
- Library thumbnails are reduced to 384 px, with two decode lanes on desktop and one on mobile. Up to 48 cached thumbnails survive closing the library for quick reopening; eviction/removal releases their URLs. The main stage's existing URL cache is separate and unchanged.
