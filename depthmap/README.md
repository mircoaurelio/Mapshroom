...
- Depth Anything V2 Small is selected by default and automatic depth estimation runs immediately after image upload. Changing the model reruns estimation when Auto Estimate is enabled.
...
- A **Depth amount** slider (0–900%, default 520%) controls relief intensity. Raw AI depth is percentile-normalized, amplified, sharpened, then stretched to full black-to-white range.
...
- **Depth Anything Small**: faster CPU-tested option for low-memory devices.
- **Depth Anything V2 Small**: default model, with newer weights and sharper relief transitions.
