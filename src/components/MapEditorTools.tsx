...
function Slider({ id, output, label, min = 0, max, value, unit = '%' }: { id: string; output: string; label: string; min?: number; max: number; value: number; unit?: string }) {
  ...
  return <div className="map-editor-field"><label htmlFor={id}>{label}<output id={output}>{value}{unit}</output></label>
  ...
  return <>
    <div className="map-editor-preview">
      <div className="map-editor-preview-toolbar"><div role="group" aria-label="Image preview" className="map-editor-segmented">
        <button type="button" data-preview-mode="100" aria-pressed="false">Original</button>
        <button type="button" data-preview-mode="50" aria-pressed="false">Compare</button>
        <button type="button" data-preview-mode="0" aria-pressed="true">Result</button>
      ...
      <div className="map-editor-preview-footer"><div className="map-editor-history" role="group" aria-label="Edit history">
        <button type="button" id="undoButton" disabled aria-label="Undo edit"><MapEditorIcon name="undo" /><span>Undo</span><kbd>Ctrl Z</kbd></button>
        <button type="button" id="redoButton" disabled aria-label="Redo edit"><MapEditorIcon name="redo" /><span>Redo</span><kbd>Ctrl Shift Z</kbd></button>
    ...
    <aside className="map-editor-inspector" aria-label="Image adjustment controls">
      <div className="map-editor-tool-tabs" role="tablist" aria-label="Image tools">
        {([{ panel: 'refine', label: 'Mask', icon: 'mask' }, { panel: 'draw', label: 'Draw', icon: 'draw' }, { panel: 'depth', label: 'Depth', icon: 'depth' }, { panel: 'wand', label: 'AI Wand', icon: 'wand' }] as const).map(tool => <button key={tool.panel} type="button" role="tab" id={`map-tab-${tool.panel}`} data-editor-panel={tool.panel} aria-controls={`${tool.panel}Panel`} aria-selected={false}><MapEditorIcon name={tool.icon} /><span>{tool.label}</span></button>)}
      </div>
      <div className="map-editor-controls">
        <section id="depthPanel" className="map-editor-tool disabled-panel" role="tabpanel" aria-labelledby="map-tab-depth">
          <h3>Depth map</h3>
          <button id="generateDepth" className="map-editor-generate" type="button" disabled><strong>Generate depth map</strong></button>
          <div className="map-editor-segmented" role="group" aria-label="Depth map color mode"><button id="depthModeBw" className="active" type="button">Black &amp; white</button><button id="depthModeRgb" type="button">Color depth</button></div>
        ...
        <section id="refinePanel" className="map-editor-tool disabled-panel" role="tabpanel" aria-labelledby="map-tab-refine">
          <h3>Mask</h3><p className="map-editor-note">Remove background areas or restore image details.</p>
          <button id="brushToggle" className="map-editor-generate" type="button">Enable brush</button>
          <div className="map-editor-segmented" role="group" aria-label="Brush action"><button id="eraseBrush" className="active" type="button">Erase</button><button id="restoreBrush" type="button">Restore</button></div>
          <Slider id="brushSizeRange" output="brushSizeValue" label="Brush size" min={8} max={400} value={80} unit=" px" />
          <button id="resetBrush" type="button">Reset brush edits</button>
          <div className="map-editor-divider" /><button id="magicErase" className="map-editor-generate" type="button"><strong>Smart erase</strong></button>
          ...
          <div className="map-editor-divider" /><h4>Edges</h4><div className="map-editor-toggle"><span>Hard mask</span><button id="hardMaskToggle" type="button" aria-pressed="false">Off</button></div>
        ...
        <section id="drawPanel" className="map-editor-tool disabled-panel" role="tabpanel" aria-labelledby="map-tab-draw">
          <h3>Draw</h3><p className="map-editor-note">Paint directly onto the image.</p><button id="drawToggle" className="map-editor-generate" type="button"><strong>Paint surface</strong></button>
          ...
          <Slider id="drawSizeRange" output="drawSizeValue" label="Brush size" min={8} max={400} value={80} unit=" px" /><button id="resetDraw" type="button">Reset drawing</button>
          <section id="cropPanel" className="map-editor-crop disabled-panel"><div className="map-editor-divider" /><h4>Crop</h4><button id="cropToggle" type="button">Enable crop</button><label className="map-editor-select">Aspect ratio<select id="cropAspect"><option value="free">Free</option><option value="1">1 : 1</option><option value="1.333333">4 : 3</option><option value="1.777778">16 : 9</option><option value="0.8">4 : 5</option></select></label>
            <div className="map-editor-toggle"><span>Output size</span><output id="cropSizeValue">—</output></div><div className="map-editor-actions"><button id="cropReset" type="button">Reset frame</button><button id="cropCancel" type="button" disabled>Cancel</button><button id="cropApply" className="map-editor-primary" type="button" disabled>Apply crop</button></div>
          </section>
        </section>
        <section id="wandPanel" className="map-editor-tool disabled-panel" role="tabpanel" aria-labelledby="map-tab-wand">
          <h3>AI Wand</h3><p className="map-editor-note">Select an area, then remove it or restore it.</p>
          <button id="wandToggle" className="map-editor-generate" type="button"><strong>Draw AI selection</strong></button>
          <div className="map-editor-segmented" role="group" aria-label="AI selection mode"><button id="wandModeMinus" className="active" type="button">Remove</button><button id="wandModePlus" type="button">Restore</button></div>
          <Slider id="wandSizeRange" output="wandSizeValue" label="Selection brush" min={12} max={320} value={64} unit=" px" />
          <div className="map-editor-actions"><button id="wandCancel" type="button" disabled>Cancel</button><button id="wandConfirm" className="map-editor-primary" type="button" disabled>Apply selection</button></div>
          <details className="map-editor-model"><summary>Background removal</summary><label className="map-editor-select" htmlFor="modelSelect">Model<select id="modelSelect" defaultValue="onnx-community/ormbg-ONNX"><option value="onnx-community/ormbg-ONNX">Objects</option><option value="Xenova/modnet">Portraits</option><option value="manual">Manual</option></select></label><span id="modelBadge" className="map-editor-model-badge" /><p id="modelDescription" className="map-editor-note" /><label className="map-editor-toggle" htmlFor="autoSegment"><span>Run on image change</span><input id="autoSegment" type="checkbox" defaultChecked={false} /></label><button id="segmentButton" className="map-editor-generate" type="button" disabled>Remove background</button></details>
        </section>
      </div>
