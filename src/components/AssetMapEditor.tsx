...
export function AssetMapEditor({ asset, assetUrl, initialPanel = 'refine', originalAsset = null, originalAssetUrl = null, onApply, onClose }: Props) {
  ...
  return <div className="map-editor-backdrop" role="presentation"><section ref={dialog} className="map-editor-dialog" role="dialog" aria-modal="true" aria-labelledby="map-editor-title" tabIndex={-1}
    ...
    <header className="map-editor-header"><div><span className="map-editor-breadcrumb">Assets / {asset.name}</span><h2 id="map-editor-title">Adjust map</h2></div><div className="map-editor-header-actions"><button type="button" className="map-editor-primary" disabled={status !== 'ready'} onClick={() => { void controller.current?.save(); }}><MapEditorIcon name="check" />{kind === 'depth' ? savedDepth ? 'Save changes' : 'Save depth map' : kind === 'draw' ? 'Save painted copy' : 'Save masked copy'}</button><button type="button" className="map-editor-close" aria-label="Close image editor" disabled={saving.current} onClick={onClose}><MapEditorIcon name="close" /></button></div></header>
